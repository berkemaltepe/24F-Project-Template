...
if st.button('Student List', use_container_width=True):
    st.switch_page('advisor_students.py')  # Redirect to Student List page
if st.button('Skill Match Analysis', use_container_width=True):
    st.switch_page('advisor_skill_match.py')  # Redirect to Skill Match Analysis page
if st.button('Active Job Postings', use_container_width=True):
    st.switch_page('advisor_job_postings.py')  # Redirect to Job Postings page
if st.button('Manage Students', use_container_width=True):
    st.switch_page('advisor_manage_students.py')  # Redirect to Manage Students page
